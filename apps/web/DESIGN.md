...
- **Layout (mobile)**: the card *is* the viewport — a flex column of illustration, copy and navigation, with `overflow: hidden` and nothing inside it scrolling. The measured contract, asserted by `e2e/onboarding.spec.ts`:
  ...
  - **one illustration box per step.** The assets are not one shape — `3onboard.webp` is 1080×810 while the other three are 810×1080 — so sizing a figure only by height rendered the third step at a visibly different footprint. The *figure* is the standard instead: `34dvh` tall on every step, `object-fit: contain` so the artwork is centred and never cropped, and `max-height: 100%` as the give. Measured identical across all four steps at 390×844 (353×287) and on desktop (598×684); on a cramped 360×640 it shrinks uniformly rather than pushing copy out of view;
